"""End-to-end pipeline test with timing logs."""
...
def log(stage: str, elapsed: float) -> None:
    print(f"  [{elapsed:6.2f}s] {stage}")


def run_flow(label: str, img: np.ndarray) -> None:
    print(f"\n{'='*60}")
    ...
    if len(faces) == 0:
        print("  -> No face detected. Pipeline stops here (expected).")
        return

    # Stage 2: Select largest face
    if len(faces) > 1:
    ...
    # Stage 4: Save to temp + SerpApi search
...
if __name__ == "__main__":
    ...
    # Test 2: single rectangle (no real face)
    single = np.zeros((480, 640, 3), dtype=np.uint8)
    cv2.rectangle(single, (200, 100), (440, 380), (120, 100, 80), -1)
    run_flow("Synthetic single object (no real face)", single)

    # Test 3: two separate objects (multiple faces scenario)
    multi = np.zeros((480, 640, 3), dtype=np.uint8)
    cv2.rectangle(multi, (50, 50), (200, 200), (120, 100, 80), -1)
    cv2.rectangle(multi, (350, 250), (580, 450), (100, 120, 140), -1)
    run_flow("Two synthetic objects (multiple detection scenario)", multi)
